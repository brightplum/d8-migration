  ...
  Drupal.behaviors.slideUpFromBottomCta = {
    attach: function (context, settings) {
      if (settings.ctas.slide_up_from_bottom) {
        // Campaign name.
        var campaign = settings.ctas.slide_up_from_bottom.campaign;

        // Clear old storage settings.
        var removeArr = [];
        for (var i = 0; i < localStorage.length; i++){
          if (localStorage.key(i).indexOf(campaign) === -1 && localStorage.key(i).indexOf('slide-up-from-bottom') !== -1) {
        ...
        var block_id = "#block-block-" + settings.ctas.slide_up_from_bottom.delta;
        var hide_banner = localStorage.getItem('commondreams.slide-up-from-bottom-' + campaign + '.hide_banner');

        // Only show after one page view.
        if (localStorage.getItem('commondreams.slide-up-from-bottom-' + campaign + '.page_views')) {
          ...
          localStorage.setItem('commondreams.slide-up-from-bottom-' + campaign + '.page_views', 1);
          hide_banner = true;
        }

        if (localStorage.getItem('commondreams.slide-up-from-bottom-' + campaign + '.force-slim')) {
        ...
        if (hide_banner) {
          ...
          $(block_id).find('#cta-handle-mobile-close').click(function(e) {
            ...
            localStorage.setItem('commondreams.slide-up-from-bottom-' + campaign + '.force-slim', true);
            $(block_id).find('.cta-handle').hide();
            e.preventDefault();
          });

          // On load, only if its not been shown.
          if (localStorage.getItem('commondreams.slide-up-from-bottom-' + campaign) !== 'shown') {
            ...
            localStorage.setItem('commondreams.slide-up-from-bottom-' + campaign, 'shown');
          }

          // Super slim.
          if (localStorage.getItem('commondreams.slide-up-from-bottom-' + campaign + '.force-slim')) {
            $(block_id).addClass('super-slim-display-showing').removeClass('slim-display-showing');
          }

          // Already donated button.
          $(block_id).find(".hide-banner").click(function(e) {
            localStorage.setItem('commondreams.slide-up-from-bottom-' + campaign + '.hide_banner', 1);
            $(block_id).remove();
            e.preventDefault();
          });
