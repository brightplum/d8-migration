...
(function($) {
  ...
  IWapi.builders.ContentBuilder.prototype = {
    ...
    pullItems: function(slot) {
      ...
      if (limit > 3) this.markBoundsAndCollapse();
    },
    ...
    pushItems: function(slot, item) {
      while (slot < 4) {
        ...
        if (!elem.length) return; // if it was empty, we're done.
    ...
    markBoundsAndCollapse: function() {
      var slot = 4, fill = -1; // starting at slot #4 we start to tag datelines.
      var date = '', day = 0;
      ...
      if (fill >= 0) {
        while (fill <= 15 && $('#edit-unused-fields .content-container > .draggable-field').length) {
          var fillStr = (fill++).toString();
  ...
  $(function() {
    ...
    $('#slot-0').prepend('<div class="section-header">Promoted</div>');
  });
